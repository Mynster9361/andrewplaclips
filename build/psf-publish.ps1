...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'andrewplaclips' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\andrewplaclips" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the andrewplaclips module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\andrewplaclips"
